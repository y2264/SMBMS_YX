...
$(function(){
	...
	$.ajax({
		type:"GET",//请求类型
		url:"/SMBMS_YX/UserServlet/getRoleList",//请求的url
	...
	userCode.bind("blur",function(){
		...
		$.ajax({
			type:"GET",//请求类型
			url:"/SMBMS_YX/UserServlet/userCodeExist",//请求的url
			data:{userCode:userCode.val()},//请求参数
			dataType:"json",//ajax接口（请求url）返回的数据类型
			success:function(data){//data：返回数据（json对象）
				if(data === "exist"){//账号已存在，错误提示
					validateTip(userCode.next(),{"color":"red"},imgNo+ " 该用户账号已存在",false);
				}else if (data === "notempty") {//账号为空，错误提示
					validateTip(userCode.next(),{"color":"red"},imgNo+" 用户编码不可以为空",false);
				}else{//账号可用，正确提示
